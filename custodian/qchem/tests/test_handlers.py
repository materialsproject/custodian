...
class QChemErrorHandlerTest(TestCase):
    ...
    def test_advanced_scf_failed_to_converge_1(self):
        ...
        inp = QCInput.from_file(os.path.join(scr_dir, "mol.qin.0"))
        from pymatgen.io.qchem.outputs import QCOutput

        out = QCOutput(os.path.join(scr_dir, "mol.qout.0")).data

        self.assertEqual(inp.rem.get("scf_algorithm", "diis").lower(), "diis")
        self.assertEqual(inp.rem.get("scf_guess_always", "none").lower(), "none")
        self.assertEqual(len(out.get("energy_trajectory", [])) > 0, True)
