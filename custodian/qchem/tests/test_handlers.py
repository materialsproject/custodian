...
class QChemErrorHandlerTest(TestCase):
    ...
    def _check_equivalent_inputs(self, input1, input2):
        self.assertEqual(
            QCInput.from_file(input1).molecule, QCInput.from_file(input2).molecule
        )
    ...
    def test_unable_to_determine_lamda(self):
        ...
        self._check_equivalent_inputs(
            "unable_to_determine_lamda.qin.0", "unable_to_determine_lamda.qin.1"
        )
    ...
    def test_scf_failed_to_converge(self):
        ...
        h = QChemErrorHandler(
            input_file="crowd_gradient.qin.0", output_file="crowd_gradient.qout.0"
        )
        h.check()
        d = h.correct()
        ...
        h = QChemErrorHandler(
            input_file="crowd_gradient.qin.1", output_file="crowd_gradient.qout.1"
        )
        h.check()
        d = h.correct()
        self.assertEqual(d["errors"], ["SCF_failed_to_converge"])
    ...
    def test_out_of_opt_cycles(self):
        ...
        h = QChemErrorHandler(
            input_file="crowd_gradient.qin.2", output_file="crowd_gradient.qout.2"
        )
        h.check()
        d = h.correct()
    ...
    def test_failed_to_read_input(self):
        ...
        h = QChemErrorHandler(
            input_file="unable_lamda_weird.qin", output_file="unable_lamda_weird.qout"
        )
        h.check()
        d = h.correct()
        ...
        self._check_equivalent_inputs(
            "unable_lamda_weird.qin.last", "unable_lamda_weird.qin"
        )
