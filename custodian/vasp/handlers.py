...
class VaspErrorHandler(ErrorHandler):
    ...
    def correct(self):
        ...
        if (self.errors.intersection(["rspher", "real_optlay", "nicht_konv"])
            and not vi["INCAR"]["LREAL"].get("LREAL", False)):
            s = vi["POSCAR"].structure
            if len(s) < self.natoms_large_cell:
                ...
                if self.error_count["real_optlay"] == 0 and vi["INCAR"]["LREAL"].get("LREAL", False) is not False:
        ...
        if "grad_not_orth" in self.errors:
            if self.error_count["grad_not_orth"] == 0:
