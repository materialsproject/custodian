...
class TestKPointUtils(PymatgenTest):
    ...
    def setUp(self) -> None:
        self.large_structure = Structure.from_file(
            os.path.join(TEST_FILES,"POSCAR_mp-1200292.gz")
        )

        self.small_structure = Structure(
            Lattice.from_parameters(a = 3.8, b = 3.8, c = 3.8, alpha = 60., beta = 60., gamma = 60.),
            ["Si","Si"],
            [[(-1)**i * 0.125 for _ in range(3)] for i in range(2)]
        )

    def test_k_point_estimate(self) -> None:
        kspacing_values = [8, 2, 1., 0.8, .6, .4, .2, .1]
        expected_kpoints_per_axis = [
            (1, 1, 1), (2, 1, 1), (3, 2, 2), (4, 3, 3), (5, 4, 4), (8, 5, 5), (15, 10, 10), (30, 20, 20)
        ]
